...
class TestIngestOperation(object):
    ...
    def test_descriptions(self, *args):
        ...
        expected = [
            ('ingest', 'class=katsdpingest.sigproc.IngestOperation', '0.0+unknown'),
            ('ingest:prepare', 'channel_range=(16, 96), channels=128, class=katsdpingest.sigproc.Prepare, in_baselines=220, out_baselines=192, scale=1.0', '0.0+unknown'),
            ('ingest:zero_spec', 'class=katsdpingest.sigproc.Zero', '0.0+unknown'),
            ('ingest:zero_spec:zero_vis', 'class=katsdpsigproc.fill.Fill, ctype=float2, dtype=complex64, shape=(80, 192), value=0j', '0.0+unknown'),
            ('ingest:zero_spec:zero_weights', 'class=katsdpsigproc.fill.Fill, ctype=float, dtype=float32, shape=(80, 192), value=0.0', '0.0+unknown'),
            ('ingest:zero_spec:zero_flags', 'class=katsdpsigproc.fill.Fill, ctype=unsigned char, dtype=uint8, shape=(80, 192), value=255', '0.0+unknown'),
            ('ingest:zero_sd_spec', 'class=katsdpingest.sigproc.Zero', '0.0+unknown'),
            ('ingest:zero_sd_spec:zero_vis', 'class=katsdpsigproc.fill.Fill, ctype=float2, dtype=complex64, shape=(80, 192), value=0j', '0.0+unknown'),
            ('ingest:zero_sd_spec:zero_weights', 'class=katsdpsigproc.fill.Fill, ctype=float, dtype=float32, shape=(80, 192), value=0.0', '0.0+unknown'),
            ('ingest:zero_sd_spec:zero_flags', 'class=katsdpsigproc.fill.Fill, ctype=unsigned char, dtype=uint8, shape=(80, 192), value=255', '0.0+unknown'),
            ('ingest:transpose_vis', 'class=katsdpsigproc.transpose.Transpose, ctype=float2, dtype=complex64, shape=(192, 128)', '0.0+unknown'),
            ('ingest:flagger', 'class=katsdpsigproc.rfi.device.FlaggerDevice', '0.0+unknown'),
            ('ingest:flagger:background', 'baselines=192, channels=128, class=katsdpsigproc.rfi.device.BackgroundMedianFilterDevice, width=13', '0.0+unknown'),
            ('ingest:flagger:transpose_deviations', 'class=katsdpsigproc.transpose.Transpose, ctype=float, dtype=float32, shape=(128, 192)', '0.0+unknown'),
            ('ingest:flagger:noise_est', 'baselines=192, channels=128, class=katsdpsigproc.rfi.device.NoiseEstMADTDevice, max_channels=10240', '0.0+unknown'),
            ('ingest:flagger:threshold', 'baselines=192, channels=128, class=katsdpsigproc.rfi.device.ThresholdSimpleDevice, flag_value=16, n_sigma=11.0, transposed=True', '0.0+unknown'),
            ('ingest:flagger:transpose_flags', 'class=katsdpsigproc.transpose.Transpose, ctype=unsigned char, dtype=uint8, shape=(192, 128)', '0.0+unknown'),
            ('ingest:accum', 'baselines=192, channel_range=(16, 96), channels=128, class=katsdpingest.sigproc.Accum, outputs=2', '0.0+unknown'),
            ('ingest:postproc', 'baselines=192, channels=80, class=katsdpingest.sigproc.Postproc, cont_factor=8', '0.0+unknown'),
            ('ingest:sd_postproc', 'baselines=192, channels=80, class=katsdpingest.sigproc.Postproc, cont_factor=16', '0.0+unknown'),
            ('ingest:timeseries', 'class=katsdpsigproc.maskedsum.MaskedSum, shape=(80, 192)', '0.0+unknown'),
            ('ingest:percentile0', 'class=katsdpsigproc.percentile.Percentile5, column_range=(0, 8), is_amplitude=False, max_columns=8, shape=(80, 192)', '0.0+unknown'),
            ('ingest:percentile0_flags', "class=katsdpsigproc.reduce.HReduce, column_range=(0, 8), ctype=unsigned char, dtype=<type 'numpy.uint8'>, extra_code=, identity=0, op=a | b, shape=(80, 192)", '0.0+unknown'),
            ('ingest:percentile1', 'class=katsdpsigproc.percentile.Percentile5, column_range=(10, 22), is_amplitude=False, max_columns=12, shape=(80, 192)', '0.0+unknown'),
            ('ingest:percentile1_flags', "class=katsdpsigproc.reduce.HReduce, column_range=(10, 22), ctype=unsigned char, dtype=<type 'numpy.uint8'>, extra_code=, identity=0, op=a | b, shape=(80, 192)", '0.0+unknown')
        ]
        self.maxDiff = None
    ...
    def run_host(self, vis, n_vis, n_sd_vis, scale, permutation,
        ...
        """
        expected = self.run_host_basic(vis[:n_vis], scale, permutation, cont_factor, channel_range, n_sigma)
        sd_expected = self.run_host_basic(vis[:n_sd_vis], scale, permutation, sd_cont_factor, channel_range, n_sigma)
        for (name, value) in sd_expected.iteritems():
            expected['sd_' + name] = value

        # Time series
        expected['timeseries'] = np.sum(expected['sd_spec_vis'] * timeseries_weights[..., np.newaxis], axis=0)
    ...
    def test_random(self, context, queue):
        ...
        data_keys = ['spec_vis', 'spec_weights', 'spec_flags', 'cont_vis', 'cont_weights', 'cont_flags']
        ...
        expected = self.run_host(vis_in, dumps, sd_dumps, scale, permutation,
