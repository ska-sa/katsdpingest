...
def autotune_device(device):
    ...
    tune_antennas = ingest_threads.CBFIngest.tune_antennas
    tune_channels = ingest_threads.CBFIngest.tune_channels
    ...
    for antennas in tune_antennas:
        ingest_threads.CBFIngest.create_proc_template(context, antennas, tune_channels[0])
    for channels in tune_channels:
        ingest_threads.CBFIngest.create_proc_template(context, tune_antennas[0], channels)
