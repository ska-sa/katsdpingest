...
void detect_and_sum (udpplot_t * ctx);
...
int main (int argc, char **argv)
{
  ...
  while ((arg=getopt(argc,argv,"c:D:F:ln:st:vz")) != -1)
  {
    switch (arg)
    {
      case 'c':
...
void detect_and_sum (udpplot_t * ctx)
{
  unsigned ipol, ichan, ipt, offset, basechan;
  ...
  for (ipol=0; ipol < ctx->npol; ipol++)
  {
    for (ichan=0; ichan < ctx->nchan_in; ichan++)
    {
      ...
      {
        float a = ctx->fft_in[ipol][ichan][(ipt*2) + 0];
        float b = ctx->fft_in[ipol][ichan][(ipt*2) + 1];
        sum += ((a*a) + (b*b));
      }
      for (ipt=0; ipt<ctx->nfft; ipt++)
      {
        ctx->y_points[ipol][basechan + ipt] += sum;
      }
    }
  }
}
