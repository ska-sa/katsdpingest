...
our $pid : shared = "";
our $r_user = "bpsr";
our $r_host = "192.168.3.11";   # caspsr-raid0 infiniband
...
Dada::logMsg(0, $dl, "STARTING SCRIPT PID=".$pid);
...
sub getBeamToSend(\@) 
{
  ...
  # find all observations that are in finished state
  $cmd = "find ".$archives_dir." -mindepth 3 -maxdepth 3 -type f -name 'beam.finished' ".
         "-printf '%h %T@\\n' | sort | awk -F/ '{print \$(NF-1)\"/\"\$NF}'";
  Dada::logMsg(2, $dl, "getBeamToSend: ".$cmd);
  ($result, $rval, $response) = Dada::mySystem($cmd);
  if ($result ne "ok")
  {
    Dada::logMsg(0, $dl, "getBeamToSend: find failed: ".$response);
    return ("fail", $host, $obs, $beam);
  }

  @beams_finished = split(/\n/, $response);
  ...
  Dada::logMsg(2, $dl, "Found ".($#beams_finished+1)." beams marked finished");

  # go through the list of finished beams, looking for something to send
  for ($i=0; (($i<=$#beams_finished) && (!$have_obs)); $i++) 
  {
    Dada::logMsg(3, $dl, "getBeamToSend: checking ".$beams_finished[$i]);
    ($o, $time) = split(/ /,$beams_finished[$i]);
    Dada::logMsg(3, $dl, "getBeamToSend: time: ".$time.", obs:".$obs);

    $obs_info_file = $archives_dir."/".$o."/obs.info";

    # skip if no obs.info exists
    if (! -f $obs_info_file)
    {
      Dada::logMsg(0, $dl, "Required file missing: ".$obs_info_file);
      next;
    }

    # get the PID
    $cmd = "grep ^PID ".$obs_info_file." | awk '{print \$2}'";
...
sub checkFullyDeleted() {
  ...
  #$cmd = "find ".$cfg{"SERVER_RESULTS_DIR"}." -mindepth 2 -maxdepth 2 -name 'obs.deleted' -printf '\%h\\n' | awk -F/ '{print \$NF}' | sort";
  ...
  for ($i=0; (($i<=$#observations) && (!$quit_daemon)); $i++) {
    ...
    $cmd = "find ".$cfg{"SERVER_RESULTS_DIR"}."/".$o." -mindepth 1 -maxdepth 1 -type l -printf '\%l\n' | awk -F/ '{print \$3}' | sort";
...
sub checkFullyArchived() {
  ...
  $cmd = "find ".$cfg{"SERVER_RESULTS_DIR"}." -mindepth 2 -maxdepth 2 -name 'obs.archived' -printf '\%h\\n' | awk -F/ '{print \$NF}' | sort";
...
sub checkFullyFinished() {
  ...
  # Find all observations marked as obs.transferred to 
  $cmd = "find ".$cfg{"SERVER_RESULTS_DIR"}."  -mindepth 2 -maxdepth 2 -name 'obs.finished' -printf '\%h\\n' | awk -F/ '{print \$NF}' | sort";
...
sub checkFullyTransferred() {
  ...
  # Find all observations marked as obs.transferred to 
  $cmd = "find ".$cfg{"SERVER_RESULTS_DIR"}."  -mindepth 2 -maxdepth 2 -name 'obs.transferred' -printf '\%h\\n' | awk -F/ '{print \$NF}' | sort";
